...
import lib.test.parameter.sttrack as rgbe_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=60, debug=0, script_name='prompt'):
    ...
    save_path = f'./RGBE_workspace/results/{dataset_name}/{save_name}_{epoch}/' + seq_txt + '.txt'
    save_folder = f'./RGBE_workspace/results/{dataset_name}/{save_name}_{epoch}/' 
    ...
    if script_name == 'sttrack':
        params = rgbe_params.parameters(yaml_name, epoch)
        ostrack = STTrack(params)  # "VisEvent"
        tracker = STTRACK_RGBE(tracker=ostrack)
...
if __name__ == '__main__':
    ...
    if dataset_name == 'VisEvent':
        seq_home = '/nasdata/tracking/data/VisEvent/test_subset'
